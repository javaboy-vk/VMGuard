...
REM     3. Only then delete the service via Procrun or sc.exe
REM
...
REM   v2.5   - Remove user shutdown tasks during uninstall (settings-driven)
REM   v2.6   - Align user task folder defaults with \Protepo\
REM   v2.7   - Treat "marked for deletion" as success after bounded retries
REM   v2.8   - Accept delete-pending state; wait longer for SCM to clear
REM   v2.9   - Rewrite as clean ASCII; fix SCM wait routine and parsing issues
REM   v3.0   - Add sc.exe delete fallback; tolerate delete-pending state
REM ============================================================
...
echo ===========================================
echo VMGuard Guard Service Uninstall v3.0
echo ===========================================
...
if errorlevel 1 (
    echo.
    echo [WARN] Procrun reported a delete failure. Checking for SCM removal...
    call :WAIT_SERVICE_REMOVAL "%SERVICE_NAME%" 15 2
    if errorlevel 1 (
        echo [WARN] Procrun delete did not clear service. Attempting sc.exe delete...
        sc delete "%SERVICE_NAME%" >nul 2>&1
        call :WAIT_SERVICE_REMOVAL "%SERVICE_NAME%" 15 2
        if errorlevel 1 (
            echo [ERROR] Service still present after delete attempts.
...
REM ==================================================================
REM STEP 5 - DELETE USER SHUTDOWN TASKS (BEST EFFORT)
REM ==================================================================
...
REM If marked for deletion, treat as success and allow SCM to finish
sc query "%SVC%" | findstr /I "MARKED FOR DELETE" >nul 2>&1
